...
def defineFCModel(networkInputLen, networkOutputLen, per_process_gpu_memory_fraction=0.85):
    ...
    with retValgraphFC.as_default() as g:
        ...
        layer = tf.layers.conv1d(layer, 32, 32, 2, padding='same', activation=tf.nn.leaky_relu)
        layer = tf.layers.conv1d(layer, 24, 24, 2, padding='same', activation=tf.nn.leaky_relu)
        layer = tf.layers.conv1d(layer, 18, 16, 2, padding='same', activation=tf.nn.leaky_relu)
        layer = tf.layers.conv1d(layer, 12, 12, 2, padding='same', activation=tf.nn.leaky_relu)
        layer = tf.layers.conv1d(layer, 9, 10, 1, padding='same', activation=tf.nn.leaky_relu)
